...
var PluginSettings = class extends import_obsidian6.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian6.Setting(containerEl).setName("Display full card height").setDesc("Auto-fit all IOC card nodes on the canvas to their content height.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.displayFullCardHeight).onChange(async (value) => {
        this.plugin.settings.displayFullCardHeight = value;
        await this.plugin.saveSettings();
        this.plugin.applyFullCardHeight(value);
      })
    );
    new import_obsidian6.Setting(containerEl).setName("Enable debug mode").setDesc("Print verbose debug messages to the browser console (F12).").addToggle(
    ...
    new import_obsidian6.Setting(containerEl).setName("GitHub repository").setDesc(createFragment((frag) => {
      frag.createEl("a", {
...
function applyFullCardHeight(app, enable) {
  const activeView = app.workspace.getActiveViewOfType(import_obsidian9.ItemView);
  if (!activeView || activeView.getViewType() !== "canvas")
    return;
  const canvas = activeView.canvas;
  if (!canvas || !canvas.nodes)
    return;
  if (DEBUG)
    console.debug("[ReduceView] applyFullCardHeight:", enable);
  canvas.nodes.forEach((node) => {
    if (!node.text)
      return;
    if (enable) {
      ...
      if (node.contentEl) {
        ...
        measuredHeight = rendered ? rendered.scrollHeight + 40 : node.contentEl.scrollHeight;
      }
...
function createIOCCard(app, iocTypeId, osType, isChild = false, autoFitHeight = false) {
...
var IOCCanvasPlugin = class extends import_obsidian13.Plugin {
  ...
  applyFullCardHeight(enable) {
    applyFullCardHeight(this.app, enable);
  }
  // ---------------------------------------------------------------
